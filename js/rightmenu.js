...
rmf.showRightMenu = function (isTrue, x = 0, y = 0) {
  ...
  // $('#rightMenu').css('left', pageX + 'px').css('top', pageY + 'px')
  if (isTrue) {
    $rightMenu.slideDown();
  } else {
    $rightMenu.hide();
...
rmf.copySelect = function () {
  ...
  //这里可以写点东西提示一下 已复制
  btf.snackbarShow('已复制选中内容，记得标记出处哦')
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function translatePage() {
    if (targetEncoding === 1) {
      ...
      translateButtonObject.children[0].classList.value = 'iconfont icon-fanti'
      translateButtonObject.children[1].innerHTML = '切换繁体'
      ...
      translateButtonObject.children[0].classList.value = 'iconfont icon-jianti'
      translateButtonObject.children[1].innerHTML = '切換簡體'
    }
  }
  function JTPYStr() {
